...
export const OfflineIndicator: React.FC = () => {
  ...
  if (!isOnline) {
    return (
      <div className="fixed bottom-4 left-4 z-50 flex items-center gap-2 rounded-xl bg-amber-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xl border border-amber-500 animate-in slide-in-from-bottom-2">
        ...
        <span>Offline Mode — Passkeys & cached security state available</span>
      </div>
    );
  }
  ...
  if (showBackOnlineToast) {
    return (
      <div className="fixed bottom-4 left-4 z-50 flex items-center gap-2 rounded-xl bg-emerald-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xl border border-emerald-500 animate-in slide-in-from-bottom-2">
        <Wifi className="w-4 h-4" />
        <span>Back Online — Firebase & Workspace Cloud connected</span>
      </div>
    );
  }
